...
The `stop_after` config field can intentionally stop at `convergence`, `extraction`, or `integration` so the runner does not require or generate later-stage commands before those physical choices are confirmed.
