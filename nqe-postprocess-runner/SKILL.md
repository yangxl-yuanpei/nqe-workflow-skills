...
- If the user only wants plots from existing `mean_force_table.csv` or `free_energy_profile.csv`, use `stop_after: plot`. Require explicit existing CSV paths, dataset label, y-column choices, unit labels, and `plot_rc_order` derived from a user-confirmed integration or initial-to-final direction. For free-energy plots, do not treat `plot_rc_order` as mere styling: if the integration direction or intended initial-to-final RC direction is not already confirmed in the current task or copied from a reviewed config, ask before writing runnable plot-only YAML. CSV headers or unit columns may be listed as observed candidates, but they are not user confirmation. If the user asks the agent to "choose" or "pick suitable" plot columns, units, or plot direction, stop and ask for confirmation instead of writing a runnable config. Do not rerun extraction/integration or fill fake TST fields just to reach plotting.
- If the user asks to reverse or change plotting direction, edit or regenerate `plot_rc_order` explicitly and dry-run again. Do not claim the direction changed after editing only `integration_direction` in a plot-only config that already has `plot_rc_order`.
