...
Dry-run review and real execution are separate approvals: a config with `parameters_confirmed: true` still needs the generated child commands to be shown and accepted before running without `--dry-run`.
...
After a real convergence-only run, fresh-agent tests should require a convergence review checklist before extraction. The checklist should summarize all windows, expand suspicious windows, list plot paths for human review, record acceptance-rate coverage/status, record user decisions such as rerun or user-approved discard, and keep candidate `per_window_skiprows_file` contents non-runnable until approved. If the user explains a decision using evidence such as a lower late-time potential-energy platform, mean-force sign change, or consistency with neighboring reaction coordinates, the agent should record that rationale without turning it into a reusable physical rule.

For CHMC/CPIHMC handoff tests, convergence plots are not enough by themselves. Fresh-agent answers should ask for or run `check_chmc_window.py` window-health coverage before declaring extraction/TI readiness, including row integrity, RC consistency, acceptance-rate source, and whether acceptance was parsed from logs, user-provided, inferred from energy deltas, or missing/TODO.

After real runner execution, fresh-agent answers should inspect `summary.json` or `plot_summary.json` and generated-output freshness before reporting numbers. If a CSV or plot may be stale, the answer should stop, report the provenance/freshness problem, and prefer a fresh output directory over destructive cleanup.

Fresh-agent tests for legacy CPIHMC outputs with `mfl`/`mfr` or other left/right mean-force components should verify that the agent does not average them automatically. The expected behavior is to diagnose both components, ask for a user-approved extraction policy, and avoid writing unsupported runnable combine fields. Newer CHMC outputs with a header and a single already averaged mean-force column should remain the main path.
