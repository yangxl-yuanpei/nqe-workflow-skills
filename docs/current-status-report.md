...
- Script-level checks on 2026-07-10 passed for Python syntax across all 14 helper scripts, runner positive dry-run, runner implicit-default refusal, and `check_chmc_window.py --print-defaults`. A follow-up read-only syntax compile check on 2026-07-17 also passed for all 14 Python helper scripts.
...
- The targeted runner output-directory boundary retest is `tests/fresh_agent_records/2026-07-17_runner-output-dir-boundary-retest_opencode.md`. It passed for non-empty `output_dir` refusal, refusing to add `allow_existing_output_dir: true` as a convenience fix, and refusing to treat runner success as clean overwrite or convergence proof.
...
- Runner stale-output negative fixtures now cover non-empty `output_dir` refusal for real execution. Dry-run still succeeds for command review, but real execution refuses before deleting or overwriting stale outputs unless `allow_existing_output_dir: true` is explicitly approved. The record is `tests/real_case_records/2026-07-17_runner_stale_output_fixture_record.md`.
...
- A plot-only fixture exists at `tests/runner_configs/demo_multi_window_plot_only.yaml`. It has been dry-run and executed to generate plots from the reviewed mean-force and free-energy CSV files without rerunning extraction, integration, or TST. The execution record is `tests/real_case_records/2026-07-12_plot_only_execution_record.md`.
...
- Real-data diagnostic records exist for a single large `PHY_QUANT` case and for a 13-window maintainer-local `../demo` CHMC/CPIHMC dataset. The raw data are intentionally outside the repository, retained record paths are sanitized for public release, and the multi-window record is `tests/real_case_records/2026-07-03_demo_multi_window_test_record.md`.
