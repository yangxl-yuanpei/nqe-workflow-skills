...
2. Extend runner negative validation beyond the current executable fixtures, especially stale partial-output handling and more real-data failure recovery. Output-directory reuse now has an executable refusal fixture, and prompt coverage for the broader cases has a targeted fresh-agent pass record.
