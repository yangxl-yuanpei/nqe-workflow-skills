...
- Treat window-health checking and convergence diagnostics as separate gates. Before saying a CHMC/CPIHMC batch is ready for extraction or TI, run or request `check_chmc_window.py` coverage for every intended window, including acceptance-rate source/status, row integrity, and RC consistency. If acceptance is missing, skipped, or inferred only from energy deltas, record that explicitly in the handoff checklist rather than silently treating convergence plots as enough.
...
- Check HMC acceptance rate and abnormal rejection behavior. If a log or user-provided acceptance value is unavailable, use the documented `KinEng`/`PotEng` fallback only when the required columns are confirmed. Report the source (`log`, `user-provided`, or `energy-delta-inferred`) and require a user-reviewed acceptance range; do not treat script defaults such as `--acceptance-threshold` as a physical pass/fail rule.
