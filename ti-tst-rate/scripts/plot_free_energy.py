#!/usr/bin/env python3
"""Plot free-energy curves from one or more free_energy_profile.csv files."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description='Plot free energy versus reaction coordinate from one or more free_energy_profile.csv files.')
    add_common_args(parser)
    parser.add_argument('--y-column', default='free_energy_au', help='Free-energy column to plot. Default: free_energy_au.')
    parser.add_argument('--free-energy-unit-label', default='', help='Optional unit label for y-axis, e.g. eV or au.')
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    ylabel = 'Free energy'
    if args.free_energy_unit_label:
        ylabel += f' ({args.free_energy_unit_label})'
    else:
        ylabel += ' (see free_energy_unit column)'
    return plot_curves(args, args.y_column, ylabel)
